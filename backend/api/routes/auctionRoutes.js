...
auctionRouter.post("/paywallet", async (req, res) => {
    ...
    if (!user_id || !amount || !auction_id) {
        return res
            .status(400)
            .json({ message: "Missing user_id, amount or auction_id" });
    }
...
auctionRouter.get("/featured", async (req, res) => {
    try {
        ...
        if (error) {
            return res
                .status(400)
                .json({ message: "Error fetching featured auctions" });
        }
...
auctionRouter.post("/create", async (req, res) => {
    try {
        ...
        return res.status(500).json({
            ...
            message:
                e.message || "An error occurred while creating the auction",
        });
    }
});
...
auctionRouter.delete("/delete", async (req, res) => {
    try {
        ...
        if (error) {
            return res
                .status(400)
                .json({ message: "Error deleting auction", error, data: data });
        }

        return res
            .status(200)
            .json({ message: "Auction deleted successfully", data });
    } catch (e) {
        console.error(e);
        return res.status(500).json({ message: "Internal server error" });
    }
...
auctionRouter.post("/updatestatus", async (req, res) => {
    ...
    if (!auction_id || !status) {
        return res
            .status(400)
            .json({ message: "Missing auction_id or status" });
    }
    ...
    try {
        ...
        if (status === "ended" && updatedAuction?.highest_bidder_id) {
            ...
            if (fetchUserError || !userData) {
                console.warn(
                    "Failed to fetch winner's user data",
                    fetchUserError,
                );
            } else {
                const { error: userUpdateError } = await supabase
                    ...
                    .update({
                        ...
                        spent_on_bids:
                            userData.spent_on_bids + updatedAuction.highest_bid,
                    })
                    .eq("user_id", updatedAuction.highest_bidder_id);

                if (userUpdateError) {
                    console.warn(
                        "Auction status updated, but failed to update winner stats",
                        userUpdateError,
                    );
                }
            }
...
auctionRouter.post("/aucdetails", async (req, res) => {
    ...
    if (!auction_id)
        return res
            .status(400)
            .json({ message: "Missing 'auction_id' in request body." });
...
auctionRouter.post("/unpaid", async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Server error while fetching unpaid auctions.",
        });
    }
});
    ...
    try {
        ...
        if (error) {
            ...
            return res
                .status(400)
                .json({
                    message: "Error favoriting auction",
                    error,
                    data: data,
                });
        }
        ...
        return res
            .status(200)
            .json({ message: "Auction favorited successfully", data });
    } catch (e) {
    ...
    try {
        ...
        if (error) {
            ...
            return res
                .status(400)
                .json({
                    message: "Error unfavoriting auction",
                    error,
                    data: data,
                });
        }
        ...
        return res
            .status(200)
            .json({ message: "Auction unfavorited successfully", data });
    } catch (e) {
...
auctionRouter.post("/bidcurrent", async (req, res) => {
    try {
        ...
        if (auctionErr || !auction) {
            ...
            return res
                .status(400)
                .json({ message: "Auction not found!", data: data });
        }
        ...
        if (isNaN(amount) || amount <= 0) {
            ...
            return res
                .status(400)
                .json({ message: "Please enter a valid bid amount" });
        }
...
auctionRouter.post("/bid", async (req, res) => {
    try {
        ...
        if (auctionErr) {
            ...
            return res
                .status(400)
                .json({ message: "Auction not found!", data: auction });
        }
        ...
        if (amount <= auction.highest_bid) {
            console.error(
                `Bid is too low. Current highest bid is ${auction.highest_bid}`,
            );
            return res
                .status(401)
                .json({
...
auctionRouter.post("/bidlow", async (req, res) => {
    try {
        ...
        if (Number(amount) >= currentLowestBid) {
            console.error(
                `Bid too high. Current lowest bid is $${currentLowestBid}`,
            );
...
auctionRouter.post("/bidhistory", async (req, res) => {
    ...
    if (!user_id) {
        return res
            .status(400)
            .json({ error: "Missing user_id in request body" });
    }
    ...
    if (error) {
        ...
        return res
            .status(500)
            .json({ error: "Failed to fetch bid history", data: data });
    }
    ...
    try {
      ...
      `,
        ...
        if (error) {
            ...
            return res
                .status(500)
                .json({ message: "Supabase query failed", error });
        }
